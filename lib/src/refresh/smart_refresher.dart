...
class SmartRefresher extends StatefulWidget {
  ...
  final SKPositionController positionController;
  ...
  /// such as ,RecordableList,doesn't allow to put into child,it will wrap it into SliverToBoxAdapter
  ...
  State<StatefulWidget> createState() {
    final state = SmartRefresherState();
    positionController.delegate = state;
    return state;
  }
}

class SmartRefresherState extends State<SmartRefresher> with SKPositionDelegate {
  ...
  SKSliverChildBuilderDelegate blockPrefix(int i,
      SKSliverChildBuilderDelegate delegate) {
  ...
  Widget _wrapScrollTag(int index, Widget child) {
    return AutoScrollTag(
      key: ValueKey(index),
      controller: widget.positionController.scrollController,
      index: index,
  ...
  List<Widget> _addRefreshSlivers(BuildContext context, List<Widget> slivers,
  ...
  List<Widget>? _buildSliversByChild(BuildContext context, Widget? child,
   ...
   return _addRefreshSlivers(context, slivers!, configuration);
  }
  ...
  Widget? _buildBodyBySlivers(
    ...
    if (childView is! Scrollable) {
      ...
      ScrollController? scrollController = widget.positionController.scrollController;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.builder != null)
      ...
      if(widget.child != null) {
        slivers = _buildSliversByChild(context, widget.child, configuration);
      }else{
        // positioned.
        final positionedSliver = SKSliverList(
          key: _k,
            delegate: widget.delegate,
          forwardRefreshCount: forwardRefreshCount,
        );

        slivers = [positionedSliver];
        _addRefreshSlivers(context, slivers, configuration);
      }
  ...
  Future<void> jumpTo(int index) async {
    ...
    _isInSecondRefresh = false;
    _k = UniqueKey();
    widget.positionController.scrollController.jumpTo(0);
    setState(() {
  ...
  Future<void> scrollTo(int index) async {
    return widget.positionController.scrollController.scrollToIndex(
