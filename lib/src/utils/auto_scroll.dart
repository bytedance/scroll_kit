...
import 'package:scroll_kit/src/adaptor/ro.dart';
...
Future<T> co<T>(Object key, FutureOr<T> action()) async {
  for (;;) {
    final c = _locks[key];
    if (c == null) break;
    try {
...
Future catchAnimationCancel(TickerFuture future) async {
  return future.orCancel.catchError((Object x) async {
...
abstract class AutoScrollController implements ScrollController {
  ...
  // ADD
  late SKRenderSliverMultiBoxAdaptor? adaptor;
...
class SimpleAutoScrollController extends ScrollController
  ...
  @override
  SKRenderSliverMultiBoxAdaptor? adaptor;
}
...
implements AutoScrollController {
  ...
  @override
  bool isIndexStateInLayoutRange(int index) => adaptor!.existChildren.contains(index);
  ...
  int? _getNearestIndex(int index) {
    final list = adaptor!.existChildren;
